        ...
        void UI_StatusBar(Texture running, Texture notRunning, Texture pingOn, Texture pingOff)
        {
            ...
            if (isPhoneSdkSettingsValid() == true)
            {
                ...
                if (isProccessRunning == true)
                {
                    ...
                    GUILayout.EndVertical();
                    EditorGUILayout.LabelField("ADB Running - 2" + androidDebuggerPreferences.sdkPath.Split(new[] { "/2" }, StringSplitOptions.None)[1], tituloBox, GUILayout.Width(20));
                }
                if (isProccessRunning == false)
                {
                    GUILayout.BeginVertical(GUILayout.Width(14));
                    ...
                    GUILayout.EndVertical();
                    EditorGUILayout.LabelField("ADB Stopped - 2" + androidDebuggerPreferences.sdkPath.Split(new[] { "/2" }, StringSplitOptions.None)[1], tituloBox, GUILayout.Width(20));
                }
                GUILayout.EndHorizontal();
            }
            if (isPhoneSdkSettingsValid() == false)
